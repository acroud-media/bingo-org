...
function blog_category_widget() {
	register_widget( 'blog_category_bingo' );
}

class blog_category_bingo extends WP_Widget {
	
	/* Register widget with WordPress. */
	
	function __construct() {
		parent::__construct(
			'blog_category_bingo', // Widget ID
			esc_html__('Blogroll 1', 'bingo'), // Name
...
<div class="blog-category">
	<ul>
		...
		if($navigation) {
			...
			$bingo_posts = new WP_Query(array( 'cat' => $categories, 'posts_per_page' => $number, 'offset' => $paged_offset, 'paged'=>$paged ));
			}else{
			$bingo_posts = new WP_Query(array( 'cat' => $categories, 'posts_per_page' => $number, 'offset' => $offset));
			}
		 while ( $bingo_posts->have_posts()) : $bingo_posts->the_post(); ?>
		<li <?php post_class((is_sticky()?'sticky':'')); ?>>		
			...
			<div class="blog-post-content">
				<?php echo nl2br(bingo_excerpt($excerptnumber)); ?>
			</div>
			<!--blog-post-content-->
		</li>
...
<p>
	<label for="<?php echo esc_attr($this->get_field_id( 'excerptnumber' )); ?>">
		<?php esc_html_e('Number of words(max: 120):', 'bingo'); ?>
	</label>
